...
export class Quaternion {
     ...
     * @param m Matrix
     */
    public quaternionToMatrix(q: Quaternion, m: any) : Mat4x4 {
    ...
    public divide(v: Quaternion | number): Quaternion {
     ...
     */
    public multiply(qa: Quaternion, qb: Quaternion): Quaternion {
        var w1: number = qa.w;
        var x1: number = qa.x;
        var y1: number = qa.y;
        var z1: number = qa.z;
    ...
    public multiplyVector(vector: Vec3, target: Quaternion | null): Quaternion {
     ...
     * @param angle angle
     */
    public fromAxisAngle(axis: Vec3, angle: number) :Quaternion {
        angle *= Math.PI / 180.0;
     ...
     * @returns 
     */
    public toAxisAngle(axis: Vec3): number {
...
;
    ...
    /**
     * Rerturns the quaternion as a rotation matrix
     * @returns 
     */
    public toRotationMatrix(): Mat4x4 {
        var x: number = this.x;
        var y: number = this.y;
        var z: number = this.z;
        var w: number = this.w;
    ...
    public getEulerAngles(eulers?: Vec3) {
        ...
        return eulers.setVec3(x, y, z).scaleBy(RADIANS_TO_DEGREES);
    }
     ...
     */
    public normalize(val: number = 1): Quaternion {
    ...
    public fromMatrix(matrix: any) {
        var v: Vec3 = matrix.decompose(Orientation.QUATERNION)[1];
        this.x = v.x;
        this.y = v.y;
        this.z = v.z;
        this.w = v.w;
     ...
     * @param target The Matrix3D object to convert the quaternion to.
     */
    public toMatrix(target: any): any {
        target ||= new Quaternion();
        target.fromQuaternion(this);
        return target;
    }
     ...
     */
    public transformVector(vector: Vec3, target: Vec3 | null): Vec3 {
        var x1: number;
        var y1: number;
        var z1: number;
        var w1: number;
     ...
     */
    public copyFrom(q: Quaternion | Vec3): this {
        var v = this;
        v.x = q.x;
        v.y = q.y;
        v.z = q.z;
    ...
    static cross(lhs: Quaternion, rhs: Quaternion, target?: Quaternion) 
    {
        let ret = target || new Quaternion();
 ...
 * @internal
 */
export function rotateVectorByQuat(lhs: Quaternion, rhs: Vec3, target?: Vec3) {
    let x = lhs.x * 2.0;
    let y = lhs.y * 2.0;
    let z = lhs.z * 2.0;
    let xx = lhs.x * x;
    ...
    let res = target ? target : new Vec3();
...
export function rotate(q: Quaternion, angle: number, v: Vec3): Quaternion {
    let tmp = v;

    // Axis of rotation must be normalized
    let len = tmp.calculateLength();
    ...
    let angleRad = angle;
    let sin = Math.sin(angleRad * 0.5);

    let rotatingQuat = new Quaternion(
    ...
    return new Quaternion().multiply(q, rotatingQuat);
}